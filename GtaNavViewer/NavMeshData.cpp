                                                         ...
                                                         std::vector<GeneratedOffmeshCandidate>* outCandidates) const
{
    ...
    if (!tile || !tile->header || localVerts.empty() || localIndices.empty()) return true;

    std::vector<float> rayVerts; std::vector<int> rayTris;
    rayVerts.reserve(localVerts.size()*3);
    for (auto& v: localVerts){ rayVerts.push_back(v.x); rayVerts.push_back(v.y); rayVerts.push_back(v.z);} 
    rayTris.reserve(localIndices.size()); for (auto i: localIndices) rayTris.push_back((int)i);

    dtNavMeshQuery* query = dtAllocNavMeshQuery(); if (!query) return false;
    const std::unique_ptr<dtNavMeshQuery, void(*)(dtNavMeshQuery*)> queryGuard(query, dtFreeNavMeshQuery);
    ...
    std::unordered_set<uint64_t> dedupe;
    auto tryAppend=[&](const GeneratedOffmeshCandidate& c,uint32_t type,size_t& bucket){ uint64_t h=HashLink(c.link.start,c.link.end,type,params.quantizePos); if(!dedupe.insert(h).second){++rejected.dedupe;return false;} ++bucket; if(outCandidates) outCandidates->push_back(c); outLinks.push_back(c.link); return true;};

    for (int polyIndex=0; polyIndex<tile->header->polyCount; ++polyIndex){
      const dtPoly* poly=&tile->polys[polyIndex]; if(poly->getType()!=DT_POLYTYPE_GROUND) continue;
      glm::vec3 polyNormal=ComputePolyNormal(tile,poly); if(polyNormal.y<slopeCos) continue; glm::vec3 polyCenter=ComputePolyCentroid(tile,poly);
      for(int edge=0; edge<poly->vertCount; ++edge){ if(!IsOpenEdge(tile,poly,edge,m_nav)) continue; ++openEdges;
        ...
        for(int sample=0; sample<params.samplesPerEdge; ++sample){ ++samples;
          ...
          glm::vec3 takeoffDrop = p - frame.outward3D * dropInset; dtPolyRef takeoffRef=0; glm::vec3 takeoffSnapped{};
          if(!SnapToNavmesh(query,takeoffDrop,snapExtents,&filter,takeoffRef,takeoffSnapped)){ ++rejected.snapFail; continue; }
          glm::vec3 sweepStart = takeoffSnapped + up*params.sweepUp;
          if(includeDrop){ glm::vec3 probe=p + frame.outward3D*dropOut + up*dropUp; glm::vec3 hit{},hitNormal{};
            if(RaycastDown(rayVerts,rayTris,probe,maxRayDistance,hit,hitNormal)){ ++dropHits; float drop=probe.y-hit.y;
              if(drop>=params.minDropThreshold && drop<=params.maxDropHeight){ if(glm::dot(hitNormal,up)>=slopeCos){ bool clear=true; if(enableSweep) clear=SweepRay3(rayVerts,rayTris,sweepStart,hit+up*params.sweepUp,params.sweepSideOffset,0.0f); else {glm::vec3 hh{},hn{}; clear=!RaycastTo(rayVerts,rayTris,sweepStart,hit+up*params.sweepUp,hh,hn);} if(clear){ dtPolyRef hr=0; glm::vec3 snapped{}; if(SnapToNavmesh(query,hit,snapExtents,&filter,hr,snapped)){ GeneratedOffmeshCandidate c{}; c.link.start=takeoffSnapped; c.link.end=snapped; c.rawStart=takeoffDrop; c.rawEnd=hit; c.link.radius=std::max(params.agentRadius,0.1f); c.link.bidirectional=false; c.link.area=params.dropArea; c.link.flags=1; c.link.userId=params.userIdBase+ (uint32_t)outLinks.size(); c.link.ownerTx=tx; c.link.ownerTy=ty; tryAppend(c,0u,dropCount);} else ++rejected.snapFail;} else {++rejected.obstruction; ++rejected.sweepBlocked;}} else ++rejected.slope;} else ++rejected.dy; }}
          if(includeJump){
            for(float d=params.minDist; d<=params.maxDist+1e-3f; d+=params.distStep){ glm::vec3 candRaw=p + frame.outwardXZ*d + frame.outward3D*jumpOut + up*jumpUp; dtPolyRef cr=0; glm::vec3 cand{}; if(!SnapToNavmesh(query,candRaw,snapExtents,&filter,cr,cand)){++rejected.snapFail; continue;} if(cr==takeoffRef){++rejected.dedupe; continue;} float dy=cand.y-takeoffSnapped.y; if(dy<-params.maxJumpDown||dy>params.maxJumpUp){++rejected.dy; continue;} bool clear= enableSweep? SweepRay3(rayVerts,rayTris,sweepStart,cand+up*params.sweepUp,params.sweepSideOffset,0.1f):true; if(!clear){glm::vec3 dir=cand-takeoffSnapped; dir.y=0; if(glm::dot(dir,dir)>1e-6f){dir=glm::normalize(dir); glm::vec3 adj=cand; dtPolyRef ar=cr; for(int it=0;it<6 && !clear;++it){adj -= dir*std::max(0.25f,params.distStep*0.5f); if(!SnapToNavmesh(query,adj,snapExtents,&filter,ar,adj) || ar==takeoffRef) break; clear= enableSweep? SweepRay3(rayVerts,rayTris,sweepStart,adj+up*params.sweepUp,params.sweepSideOffset,0.1f):true; if(clear){cand=adj; cr=ar;}}}} if(!clear){++rejected.obstruction; continue;} GeneratedOffmeshCandidate c{}; c.link.start=takeoffSnapped; c.link.end=cand; c.rawStart=p-frame.outward3D*jumpInset; c.rawEnd=candRaw; c.link.radius=std::max(params.agentRadius,0.1f); c.link.bidirectional=true; c.link.area=params.jumpArea; c.link.flags=1; c.link.userId=params.userIdBase+(uint32_t)outLinks.size(); c.link.ownerTx=tx; c.link.ownerTy=ty; if(tryAppend(c,1u,jumpCount)) break; }
          }
          if(includeClimb){ float minH=params.climbMinHeight; float maxH=params.climbMaxHeight>0?params.climbMaxHeight:params.maxJumpUp;
            for(float d=params.minDist; d<=params.maxDist+1e-3f; d+=params.distStep){ glm::vec3 front = p + frame.outwardXZ*d + frame.outward3D*climbOut + up*params.maxJumpUp; glm::vec3 topHit{},topN{}; if(!RaycastDown(rayVerts,rayTris,front, std::max(params.climbProbeDown,0.25f)+params.maxJumpUp, topHit, topN)){++rejected.snapFail; continue;} dtPolyRef cr=0; glm::vec3 cand{}; if(!SnapToNavmesh(query,topHit,snapExtents,&filter,cr,cand)){++rejected.snapFail; continue;} float dy=cand.y-takeoffSnapped.y; if(dy<minH || dy>maxH){++rejected.dy; continue;} GeneratedOffmeshCandidate c{}; c.link.start=takeoffSnapped; c.link.end=cand; c.rawStart=p-frame.outward3D*climbInset; c.rawEnd=front; c.link.radius=std::max(params.agentRadius,0.1f); c.link.bidirectional=false; c.link.area=params.climbArea; c.link.flags=1; c.link.userId=params.userIdBase+(uint32_t)outLinks.size(); c.link.ownerTx=tx; c.link.ownerTy=ty; if(tryAppend(c,2u,climbCount)) break; }
          }
        }
      }
    }
    printf("[AutoOffmeshV2][tile %d,%d] openEdges=%zu samples=%zu dropHits=%zu snapFail=%zu sweepBlocked=%zu accepted(drop/jump/climb)=%zu/%zu/%zu cap=%d\n",
           tx, ty, openEdges, samples, dropHits, rejected.snapFail, rejected.sweepBlocked, dropCount, jumpCount, climbCount, params.maxLinksPerTile);
    return true;
}
