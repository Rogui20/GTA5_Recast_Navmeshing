...
namespace
{
    ...
    glm::vec3 EdgeOutwardXZ(const glm::vec3& a,
                            ...
                            const glm::vec3& polyCenter)
    {
        return ComputeEdgeOutwardNormal(a, b, polyCenter);
    }
    ...
    for (int tileIndex = 0; tileIndex < maxTiles; ++tileIndex)
    {
        ...
        for (int polyIndex = 0; polyIndex < tile->header->polyCount; ++polyIndex)
        {
            ...
            for (int edge = 0; edge < poly->vertCount; ++edge)
            {
                ...
                glm::vec3 outward = EdgeOutwardXZ(a, b, polyCenter);
                ...
                for (int sample = 0; sample < params.samplesPerEdge; ++sample)
                {
                    ...
                    const glm::vec3 sweepStart = takeoff + up * params.sweepUp;
                    ...
                    if (includeJump || includeClimb)
                    {
                        for (float d = params.minDist; d <= params.maxDist + 1e-3f; d += params.distStep)
                        {
                            ...
                            const glm::vec2 deltaXZ(cand.x - takeoff.x, cand.z - takeoff.z);
                            ...
                            const float dy = cand.y - takeoff.y;
                            bool accept = false;
                            uint32_t type = 1u;
                            ...
                            bool clear = true;
                            const glm::vec3 sweepEnd = cand + up * params.sweepUp;
                            if (enableSweep)
                            {
                                clear = SweepRay3(m_cachedVerts, m_cachedTris, sweepStart, sweepEnd,
                                                  params.sweepSideOffset, 0.0f);
                            }
                            else
                            {
                                glm::vec3 hit{}, normal{};
                                clear = !RaycastTo(m_cachedVerts, m_cachedTris, sweepStart, sweepEnd, hit, normal);
                            }
